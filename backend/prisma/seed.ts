...
async function main() {
  ...
  await createShirtProduct({
    ...
    // 2026-09-15.
    //
    // תיקון bidi (2026-09-15, נבדק בפרודקשן ע"י אורן אחרי ה-seed/deploy
    // הראשון): בניגוד להערכה הראשונית שלי (שהנחתי שאלגוריתם ה-bidi הרגיל
    // (UAX#9) יציג "45 Grave" נכון בלי טיפול מיוחד, מכיוון שזה run אחד
    // רציף) - אורן דיווח בפועל שבעברית זה כן מוצג הפוך ("Grave" לפני "45").
    // ההערכה שלי הייתה שגויה - כנראה כי ספרות ("45") הן טיפוס bidi חלש
    // (European Number) שיכול "להיצמד" לצד הלא-נכון של הרצף המעורב-כיוון
    // כשאין הפרדה מפורשת. התיקון: עוטפים את "45 Grave" (רק בתוך ה-string
    // הזה, רק ב-*he*, לא ב-en) ב-Unicode directional isolate מפורש -
    // U+2066 (LRI, "Left-to-Right Isolate") ... U+2069 (PDI, "Pop
    // Directional Isolate") - כדי לאלץ תצוגה משמאל-לימין פנימית בלי תלות
    // בהקשר סביב. תווים בלתי-נראים לגמרי (zero-width), נתמכים בכל דפדפן/
    // מנוע רינדור מודרני (כולל לקוחות מייל, רלוונטי כי אותו he name זורם
    // גם ל-productNameSnapshot במייל אישור הזמנה). "בעברית בלבד" מתקיים
    // ממילא - זה שדה `he` נפרד מ-`en`, לא נגיעה גורפת בקוד/markup.
    //
    // יש עוד מופע נפרד של "45 Grave" בעייתי - bandCreditName, שמוזרק inline
    // לתוך משפט עברי דרך <Trans> ב-
    // frontend/src/components/product/BandCredit.tsx (<span lang="en"> בלי
    // dir="ltr" מפורש). זה שדה משותף (לא מפוצל en/he) ומוצג רק דרך קומפוננטת
    // frontend - לכן התיקון שם הוא markup (dir="ltr" על אותו span), לא
    // data - הועבר לשיחת הפרונטאנד בנפרד, לא טופל כאן.
