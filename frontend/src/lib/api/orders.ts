...
// דורש login (cookie) - docs/API_CONTRACT.md. שים לב: ה-endpoint כאן הוא
// /api/orders (לא /orders - זה ה-URL של עמוד ה-SPA עצמו, ראו docs/PRD.md
// סעיף 66). אם ה-session לא בתוקף בפועל
...
// "GET /api/orders" / "GET /api/orders/:id" (prefix /api שונה מ-URL העמוד
// עצמו ב-SPA, /orders - docs/PRD.md סעיף 66, תיקון להתנגשות routes). הקוראים (OrdersPage/OrderDetailPage) לא
...
export function getOrders(signal?: AbortSignal) {
  return api.get<OrdersListResponse>("/api/orders", { signal });
}

export function getOrder(orderId: string, signal?: AbortSignal) {
  return api.get<OrderDetailResponse>(`/api/orders/${orderId}`, { signal });
}
